...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        if self.model is not None and hasattr(
            ...
            if (
                getattr(self.model.config, "model_type", None) == "qwen2_5_omni_thinker"
            ):  # for qwen2omni
                ...
                delta0 = (
                    (1 - rope_index_kwargs["attention_mask"]).sum(dim=-1).unsqueeze(1)
                )
                # avoid conflict
                new_position_ids, rope_deltas = self.model.get_rope_index(
                    **rope_index_kwargs
                )
